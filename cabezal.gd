...
# Referencias a los prefabs (tienes que guardar tus cilindros como escenas .tscn)
@export var ficha_1_scene: PackedScene
@export var ficha_0_scene: PackedScene
...
func logica_resta(simbolo, objeto_fisico):
	match estado_actual:
		...
		Estado.Q3: # En A, borrar un 1
			...
			elif simbolo == "0": finalizar() # Error o A<B (no debería pasar según reglas)
...
func borrar_ficha(objeto):
	if objeto:
		# Opción A: Desaparecer (Simple)
		objeto.queue_free()
		
		# Opción B (Física): Empujarla fuera (Más divertido y visual)
		# objeto.apply_central_impulse(Vector3(0, 2, 5)) # Sale volando
